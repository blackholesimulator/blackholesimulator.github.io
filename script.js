...
const LensingShader = {
    ...
    fragmentShader: `
        ...
        void main() {
            ...
            if (distanceToCenter <= eventHorizonRadius) discard;
...
function buildStars() {
    ...
    scene.add(new THREE.Points(geo, new THREE.PointsMaterial({ size: 1.1, color: 0xffffff, transparent: true, opacity: 0.8 })));
}

function buildBlackHole() {
    const lensingGeo = new THREE.SphereGeometry(20.0, 64, 64);
    lensingMesh = new THREE.ShaderMaterial({
        uniforms: { uMass: { value: State.mass }, uLensingStrength: { value: State.lensing }, uTime: { value: 0.0 } },
    ...
    diskMesh = new THREE.ShaderMaterial({
        uniforms: { uTime: { value: 0.0 }, uIntensity: { value: State.diskIntensity }, uTurbulence: { value: State.turbulence } },
...
function buildParticles() {
    ...
    particleSystem = new THREE.Points(geo, new THREE.ShaderMaterial({
        uniforms: { uTime: { value: 0.0 }, uSuction: { value: State.suction }, uStretch: { value: State.stretch }, uMass: { value: State.mass } },
        vertexShader: `
            uniform float uTime; uniform float uSuction; uniform float uStretch; uniform float uMass;
            attribute vec3 aMetrics; varying float vStretch;
            void main() {
                vec3 pos = position;
                float r = aMetrics.x; float speed = aMetrics.z;
                float angle = aMetrics.y - (uTime * speed * 0.8);
                float dr = r - mod(uTime * uSuction * 0.5, r - (uMass * 0.4));
                if(dr < (uMass * 0.4)) dr = 30.0;
                pos.x = dr * cos(angle); pos.z = dr * sin(angle);
                vStretch = (1.0 / (dr - (uMass * 0.3))) * uStretch;
                vec4 mv = modelViewMatrix * vec4(pos, 1.0);
                gl_PointSize = (2.5 * (300.0 / -mv.z)) * clamp(vStretch * 0.4, 1.0, 5.0);
        ...
        fragmentShader: `
            ...
            void main() {
                ...
                gl_FragColor = vec4(mix(vec3(1.0,0.35,0.0), vec3(1.0,0.95,0.8), vStretch*0.2), 0.75);
            }
        `,
...
function setupUIEvents() {
    document.getElementById('param-mass').addEventListener('input', (e) => {
        ...
        State.mass = v;
        lensingMesh.uniforms.uMass.value = v;
        particleSystem.material.uniforms.uMass.value = v;
        document.getElementById('v-mass').innerText = v.toFixed(2) + 'M';
    });
    document.getElementById('btn-restart-sequence').addEventListener('click', resetCameraTrajectory);
}
