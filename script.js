...
function buildParticles() {
    ...
    particleSystem = new THREE.Points(geo, new THREE.ShaderMaterial({ 
        ...
        vertexShader: `uniform float uTime; uniform float uSuction; uniform float uStretch; uniform float uMass; attribute vec3 aMetrics; varying float vStretch; void main() { vec3 pos = position; float r = aMetrics.x; float speed = aMetrics.z; float angle = aMetrics.y - (uTime * speed * 0.8); float dr = r - mod(uTime * uSuction * 0.5, r - (uMass * 0.4)); if(dr < (uMass * 0.4)) dr = 30.0; pos.x = dr * cos(angle); pos.z = dr * sin(angle); vStretch = (1.0 / (dr - (uMass * 0.3))) * uStretch; vec4 mv = modelViewMatrix * vec4(pos, 1.0); gl_PointSize = (2.5 * (300.0 / -mv.z)) * clamp(vStretch * 0.4, 1.0, 5.0); gl_Position = projectionMatrix * mv; }`, 
        fragmentShader: `varying float vStretch; void main() { vec2 c = gl_PointCoord - vec2(0.5); if(length(c) > 0.5) discard; gl_FragColor = vec4(mix(vec3(1.0,0.35,0.0), vec3(1.0,0.95,0.8), vStretch*0.2), 0.75); }`, 
