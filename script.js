...
const LensingShader = {
    ...
    fragmentShader: `uniform float uMass; uniform float uLensingStrength; uniform float uTime; varying vec2 vUv; varying vec3 vWorldPosition; void main() { vec3 relPos = vWorldPosition - vec3(0.0); float distanceToCenter = length(relPos); float eventHorizonRadius = uMass * 0.8; if (distanceToCenter <= eventHorizonRadius) discard; float deflection = (uLensingStrength * uMass) / (distanceToCenter - eventHorizonRadius); float edgeGlow = pow(eventHorizonRadius / distanceToCenter, 4.5); vec3 horizonGlow = vec3(0.0, 0.75, 1.0) * edgeGlow * (1.6 + sin(uTime * 2.0) * 0.25); gl_FragColor = vec4(horizonGlow, clamp(edgeGlow + (deflection * 0.12), 0.0, 1.0)); }`
};
...
function buildBlackHole() {
    const lensingGeo = new THREE.SphereGeometry(8.0, 64, 64);
    ...
    const diskGeo = new THREE.PlaneGeometry(150, 150);
